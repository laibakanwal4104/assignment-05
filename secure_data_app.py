...
if choice == "Home":
    ...
    st.markdown('''Develop a Streamlit-based secure data storage and retrievel system where: Users store
    ...
    if st.button("Register"):
        ...
        if st.button("Login"):
            if username in stored_data and stored_data[username]["password"] == hash_password(password):
                ...
                st.error(f"❌ Invalid Credemtials! Attempts left: {remaining}")
    ...
    if not st.session_state.authenticated_user:
        ...
        if st.button("Encrypt And Save"):
            if data and passkey:
                ...
                save_data(stored_data)
                st.success("✅ Data ancrypted and save sucessfully !")

            else:
                st.error("All fields are required to fill.")

# === data retieve data section ===
elif choice == "Reteive Data":
    if not st.session_state.authenticatetd_user:
        st.warning("🔓Please login first ")
    else:
